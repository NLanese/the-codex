const Styles = {
    Fonts: {
        pageTitle: {fontFamily: "Gilroy", fontSize: 32, fontWeight: 700, textAlignVertical: 'center'},
        h1: {fontFamily: "Gilroy", fontSize: 24, fontWeight: 600},
        h2: {fontFamily: "Gilroy", fontSize: 20, fontWeight: 500, textAlignVertical: 'center'},
        
        definition: {fontFamily: "Gilroy", fontSize: 16, fontWeight: 300, paddingRight: 5, paddingLeft: 5, paddingTop: 3, paddingBottom: 3},
    },

}

export default Styles